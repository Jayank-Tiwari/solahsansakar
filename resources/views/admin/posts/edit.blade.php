    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
        <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('title', $post->title) }}" required>
            </div>

            <div class="mb-4">
                <label for="excerpt" class="block font-semibold text-gray-700">Excerpt</label>
                <textarea name="excerpt" id="excerpt" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>{{ old('excerpt', $post->excerpt) }}</textarea>
            </div>

            <div class="mb-4">
                <label for="body" class="block font-semibold text-gray-700">Full Content</label>
                <textarea name="body" id="body" rows="10" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>{{ old('body', $post->body) }}</textarea>
            </div>
            ...
            <div class="mt-6">
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded-lg">Update Post</button>
            </div>
        </form>
    </div>
</x-layouts.admin>